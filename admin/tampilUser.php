...
<body>
	...
	<div class="row" style="padding-top: 60px;">
		...
		<div class="col-md-10">
			...
			<h2><span class="glyphicon glyphicon-user"></span> Tampil Data User</h2><br>
		</div>
		<div class="col-md-1"></div>
	</div>
				...
				<p class="text">
					...
					<form method="post" action="tampilUser.php">
						<div class="form-group">
							<input type="text" name="Search" placeholder="Nama atau email User" class="form-control" style="width: 30%; float: left">&nbsp; 
				...
				<table class="table table-striped" style="font-size: 12px">
					<tr>
						...
						<?php
							...
							$result = mysqli_query($koneksi, "SELECT * FROM tbuser");
							$total = mysqli_num_rows($result);
							$pages = ceil($total/$halaman);

							//search
							if(!isset($_POST["Search"])){
								$query = mysqli_query($koneksi, "SELECT IdUser, Email, Nama, TglLahir, JK, NoTelp, Alamat FROM tbuser LIMIT $mulai, $halaman");
							}else{
								$cari = $_POST["Search"];
								$query = mysqli_query($koneksi, "SELECT IdUser, Email, Nama, TglLahir, JK, NoTelp, Alamat FROM tbuser WHERE Nama LIKE '%$cari%' OR Email LIKE '%$cari%' LIMIT $mulai, $halaman");
							}
							...
							while($data = mysqli_fetch_array($query)){
								...
								<td>".$data[6]."</td>
								<td>
									<a href='editUser.php?id=$data[0]'>
										<span class='glyphicon glyphicon-cog'></span>
									</a> | 
									<a href='hapusUser.php?id=$data[0]' onclick='return valDelete();'>
